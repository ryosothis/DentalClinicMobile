            ...
            try
            {
                ...
                if (medicalHistory.Rows.Count > 0)
                {
                    foreach (DataRow record in medicalHistory.Rows)
                    {
                        ...
                        string diagnosis = record["diagnosis"]?.ToString() ?? "Не установлен";
                        ...
                        leftStack.Add(new Label { Text = "Установленный диагноз", TextColor = Color.FromArgb("#828CA0"), FontSize = 10, FontAttributes = FontAttributes.Bold });
                        ...
                        rightStack.Add(new Label { Text = "Дата визита", TextColor = Color.FromArgb("#828CA0"), FontSize = 11, FontAttributes = FontAttributes.Bold });
                ...
                else
                {
                    ...
                    {
                        Text = "История записей отсутствует",
        ...
        private async void OnEditNameClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(lastName) && !string.IsNullOrWhiteSpace(firstName))
                {
                    ...
                    if (success)
                    {
                        await DisplayAlert("Успех", "Имя успешно обновлено", "OK");
                        string fullName = $"{lastName} {firstName} {middleName}".Trim();
                        FullNameTextBox.Text = fullName;
                    ...
                    {
                        await DisplayAlert("Ошибка", "Ошибка при обновлении имени", "OK");
                    }
                }
            }
            catch (Exception ex)
        ...
        private async void OnEditEmailClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(newEmail) && newEmail != currentEmail)
                {
                    ...
                    {
                        await DisplayAlert("Ошибка", "Некорректный email", "OK");
                        return;
                    }
        ...
        private async void OnEditPhoneClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                if (newPhone != null && newPhone != currentPhone)
                {
                    ...
                    if (success)
                    {
                        await DisplayAlert("Успех", "Телефон успешно изменен", "OK");
